...
G4MaterialPropertiesTable* OpticalMaterialProperties::FusedSilica()
{
  ...
  // REFRACTIVE INDEX
  // Sellmeier's equation for fused silica is valid up to ~10.7 eV,
  // but we're defining the refractive index only in our (smaller) range.
...
G4MaterialPropertiesTable* OpticalMaterialProperties::TPB_LXe_nconst(G4double decay)
{
  ...
  const G4int rIndex_numEntries = 2;
  G4double rIndex_energies[rIndex_numEntries] = {optPhotMinE_, optPhotMaxE_};
  G4double TPB_rIndex[rIndex_numEntries]      = {1.7, 1.7};
  mpt->AddProperty("RINDEX", rIndex_energies, TPB_rIndex, rIndex_numEntries);
